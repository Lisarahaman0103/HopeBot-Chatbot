...
def clean_response(text):
    ...
    text = html.unescape(text)  # Fix HTML-like characters
    ...
    text = re.sub(r"[^\x00-\x7F]+", "'", text)  # buang unicode pelik
    text = re.sub(r"([a-z])([A-Z])", r"\1. \2", text)  # pisah ayat sambung
    ...
    if len(sentences) > 2:
        text = " ".join(sentences[:2])  # Ambil 2 ayat sahaja
...
def generate_gpt2_response(user_input):
    ...
    prompt = f"Provide supportive advice related to {predicted_label}.\nUser: {user_input}\nHopeBot:"
    ...
    if len(response.split()) < 5:
        response = "I'm here for you. You're not alone, and it's okay to feel like this."
    if '"' in response:
        response = response.split('"')[0].strip()
    if response.lower().startswith("hopebot:"):
        response = response[8:].strip()

    return f"**(Topic: {predicted_label})**\n\n{response}"
